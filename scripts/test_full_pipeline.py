...
for i, item in enumerate(test_suite, 1):
    ...
    print(f"[{i:02d}/{len(test_suite)}] {item['cat']} ({item['lang']}) -> Intent: {res['detected_intent']} | Sent: {res['detected_sentiment']} | Esc: {res['priority_escalation']} | Route: {res['routing_action']}")
